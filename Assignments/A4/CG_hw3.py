...
""" 
Function: GetLine3D
Description: Read Line from SMF File 
Arguments: none
Return: segments[]
"""
def getLine3D(inFile):
    postFile = open(inFile, 'r')
    readlines = postFile.readlines()
    segments= []
    
    # Read lines in file and save as ______________
    for l in readlines:
        tmp = l.strip().split(" ")
        if ((tmp[0] == 'v') and (len(l.strip()) != 0)):
            print(l)
            # line = (l.strip().split(" "))
            # while "" in line:
            #     line.remove("")
...
def scanfill(polygons, buffer):
    ...
    for polygon in polygons:
        ...
        # scanlist = [[]]*(ymax-ymin)
        scanlist = [[] for _ in range(ymax-ymin)]
        ...
        for edge in polygon:
            if edge[1] == edge[3]:  # If edge is horizontal
                ...
                while y < yTop:
                    ...
                    scanlist[(y-ymin)].append(x)
                    y+=1

            
        
        # Sort each Scanline X values
        yPos = 0
        while yPos+ymin < ymax:
            l = scanlist[yPos]
            l.sort()
            ...
            if (len(l)%2) == 0:
                ...
                while q >= 0:
                    start = int(l[q-2])
                    end = int(math.ceil(l[q-1]))
                    while start <= end+1:
                        try:
                            buffer[-(ymin + yPos)][start]=1
                            start += 1
                        except IndexError:
                            start += 1              
                    q -= 2
